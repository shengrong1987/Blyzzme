...
- (void)viewDidLoad
{
    ...
    [self.navigationItem setTitle:NSLocalizedString(@"titleShare", nil)];
    [self.navigationItem.rightBarButtonItem = [UIBarButtonItem alloc]initWithTitle:NSLocalizedString(@"btShare", nil) style:UIBarButtonItemStylePlain target:self action:@selector(post)];
    /********/
...
{
    static NSString *CellIdentifer = @"DescrCellId";
    
    if (IS_TABLE_HASHTAG) {
        UITableViewCell *cell;
        
        if(isTypeHashtag) {
            CellIdentifer = @"SearchCellId";
            ...
            //User cell
            CellIdentifer = @"UserCellId";
            cell = (UITableViewCell *)[tableView dequeueReusableCellWithIdentifier:CellIdentifer forIndexPath:indexPath];
    ...
    switch (indexPath.row) {
        case 0: {
            ...
            [textView setPlaceholder:NSLocalizedString(@"Say something...", nil) ];
...
-(void)textViewDidChange:(UITextView *)textView {
    ...
    [lblCounter setText:[NSString stringWithFormat:@"%lu",POST_DESCR_MAX_LENGHT-textView.text.length]];
...
{    
    //Send logout online
    NSString *descr = ((SSTextView *)[[tb cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:0]] viewWithTag:TAG_TEXTVIEW]).text;
    UIImageView *imgView = (UIImageView *)[[tb cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:0]] viewWithTag:TAG_IMAGE];
    
    if([descr length] < POST_DESCR_MIN_LENGHT) {
...
//    if(postHaveImage){
//        UIImage *image = [Utility scaleImage:[UIImage imageNamed:@"follower_normal.jpg"] toSize:CGSizeMake(320, 320) ];
//        img_data = UIImageJPEGRepresentation(image, 1.0);
//        [photos addObject:img_data];
//        
//        image = [Utility scaleImage:[UIImage imageNamed:@"follower_selected.jpg"] toSize:CGSizeMake(320, 320) ];
//        img_data = UIImageJPEGRepresentation(image, 1.0);
//        [photos addObject:img_data];
//        [[DataManager getInstance] postWithMutilImages:descr imageData:photos mediaLink:link];
//    }
    #pragma test end
    NSMutableArray *photos = [[[NSMutableArray alloc]init] autorelease];
    if(postHaveImage) {
        UIImage *image = [Utility scaleImage:imgView.image toSize:CGSizeMake(640, 640) ];
        img_data = UIImageJPEGRepresentation(image, 1.0);
        [photos addObject:img_data];
    }
...
-(void) imagePickerController:(DLCImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    if (info) {
        ...
        UIButton *btDelImage = (UIButton *)[[tb cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:0]] viewWithTag:TAG_BUTTON_DELETE_IMAGE];
