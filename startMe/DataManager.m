...
- (NSMutableArray *)getNotificationArray {
    ...
    for(NSDictionary *n in arr) {
        Notification *notific = [[[Notification alloc] initWithDictonary:n] autorelease];
            ...
            if([logged isEqualToString:@"1"]) {
                ...
                if([new_notifications count] > 0) {
                    ...
                    if([new_notifications count] >= MAX_SAVED_NOTIFICATIONS) {
                        ...
                        int len = (int)(MAX_SAVED_NOTIFICATIONS-([new_notifications count]+[dictNotifications count]));
                    ...
                    for(int i=(int)[new_notifications count]-1; i>=0; i--) {
                        ...
                        if([match count] > 0) {
                            if(!notific.is_new) {
                                ...
                                int notificationIndex = (int)[dictNotifications indexOfObject:[match objectAtIndex:0]];
...
-(void)postWithMutilImages:(NSString *)descr imageData:(NSMutableArray *)imageDatas mediaLink:(NSString *)mediaLink{
    ...
    if(imageDatas && [imageDatas count]>0){
        for (NSData *imgData in imageDatas) {
            if(imgIndex==0)
                [request setData:imgData withFileName:[NSString stringWithFormat:@"image_%d",imgIndex] andContentType:@"image/jpeg" forKey:[NSString stringWithFormat:@"photo_%d",imgIndex]];
            else
                [request addData:imgData withFileName:[NSString stringWithFormat:@"image_%d",imgIndex] andContentType:@"image/jpeg" forKey:[NSString stringWithFormat:@"photo_%d",imgIndex]];
            imgIndex++;
        }
    }
    [request setDelegate:self];
