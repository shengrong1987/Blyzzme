...
- (void)setUpLabel
{
    ...
    _colorHashtag = [[UIColor alloc] initWithRed:17.0/255.0 green:117.0/255.0 blue:186.0/255.0 alpha:1.0];
	_colorLink = [[UIColor alloc] initWithWhite:170.0/255.0 alpha:1.0];//[[UIColor alloc] initWithRed:129.0/255.0 green:171.0/255.0 blue:193.0/255.0 alpha:1.0];
}
...
- (void)drawTextInRect:(CGRect)rect
{
    ...
    CGSize sizeSpace = [@" " sizeWithMyFont:self.font constrainedToSize:rect.size lineBreakMode:self.lineBreakMode];
    ...
    for (int repeat = 0; repeat < 2; repeat++)
    {
        for (NSString *wordArray in words)
        {
            NSString *word = @"";
            NSString *lastPrefix = @"";
            NSMutableString *alreadyPrintedWord = [[[NSMutableString alloc] init] autorelease];
            NSMutableString *printedTouchableWord = [[[NSMutableString alloc] init] autorelease];
            ...
            do
            {
                ...
                sizeWord = [word sizeWithMyFont:self.font];
                ...
                if (![preCharacters isEqualToString:@""])
                {
                    ...
                    if (self.shadowColor != NULL)
                    {
                        ...
                        if (repeat)
                        {
                            [preCharacters drawAtPoint:CGPointMake(drawPoint.x + self.shadowOffset.width, drawPoint.y + self.shadowOffset.height) withAttributes:@{NSFontAttributeName:self.font}];
                        }
                    }
                    
                    [self.textColor set];
                    CGSize sizePreCharacters = [preCharacters sizeWithMyFont:self.font];
                    
                    if (repeat)
                    {
                        [preCharacters drawAtPoint:drawPoint withAttributes:@{NSFontAttributeName: self.font}];
                    }
                ...
                if (![wordCharacters isEqualToString:@""])
                {
                    ...
                    if (self.shadowColor != NULL)
                    {
                        ...
                        if (repeat)
                        {
                            [wordCharacters drawAtPoint:CGPointMake(drawPoint.x + self.shadowOffset.width, drawPoint.y + self.shadowOffset.height) withAttributes:@{NSFontAttributeName: self.font}];
                        }
                    }
                    ...
                    CGSize sizeWordCharacters = [wordCharacters sizeWithMyFont:self.font];
                    
                    if (repeat)
                    {
                        [wordCharacters drawAtPoint:drawPoint withAttributes:@{NSFontAttributeName:self.font}];
                    }
                ...
                if (![postCharacters isEqualToString:@""])
                {
                    ...
                    if (matchNewLine)
                    {
                        ...
                        if (self.shadowColor != NULL)
                        {
                            ...
                            if (repeat)
                            {
                                [[postCharacters substringToIndex:matchNewLine.range.location] drawAtPoint:CGPointMake(drawPoint.x + self.shadowOffset.width, drawPoint.y + self.shadowOffset.height) withAttributes:@{NSFontAttributeName:self.font}];
                            }
                        }
                        ...
                        if (repeat)
                        {
                            [[postCharacters substringToIndex:matchNewLine.range.location] drawAtPoint:drawPoint withAttributes:@{NSFontAttributeName: self.font}];
                        }
                        ...
                        if (!repeat)
                        {
                            ...
                            switch (_horizontalAlignment) {
                                case STHorizontalAlignmentLeft:
                                    ...
                                    break;
                                case STHorizontalAlignmentCenter:
                                    newHAlignment = (rect.size.width - drawPoint.x - [[postCharacters substringToIndex:matchNewLine.range.location] sizeWithMyFont:self.font].width) / 2;
                                    break;
                                case STHorizontalAlignmentRight:
                                    newHAlignment = rect.size.width - drawPoint.x - [[postCharacters substringToIndex:matchNewLine.range.location] sizeWithMyFont:self.font].width;
                                    break;
                                default:
                                    break;
                            }
                    ...
                    else
                    {
                        ...
                        if (self.shadowColor != NULL)
                        {
                            ...
                            if (repeat)
                            {
                                [postCharacters drawAtPoint:CGPointMake(drawPoint.x + self.shadowOffset.width, drawPoint.y + self.shadowOffset.height) withAttributes:@{NSFontAttributeName:self.font}];
                            }
                        }
                        ...
                        CGSize sizePostCharacters = [postCharacters sizeWithMyFont:self.font];
                        
                        if (repeat)
                        {
                            [postCharacters drawAtPoint:drawPoint withAttributes:@{NSFontAttributeName:self.font}];
                        }
...
- (NSString *)cutTheString:(NSString *)word atPoint:(CGPoint)drawPoint
{
    ...
    for (int i = 1; i < [word length]; i++)
    {
        ...
        CGSize sizeSubstring = [substring sizeWithMyFont:self.font];
