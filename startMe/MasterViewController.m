...
-(void)initializeView {
    ...
    NSMutableAttributedString *titleString = [[NSMutableAttributedString alloc]initWithString:@"reset password"];
    ...
    [btResetPass setAttributedTitle:titleString forState:UIControlStateNormal];
    
    [titleString release];
    
    NSMutableAttributedString *titleString2 = [[NSMutableAttributedString alloc]initWithString:@"CREATE NEW ACCOUNT"];
    ...
    [titleString2 addAttribute:NSForegroundColorAttributeName value:[UIColor whiteColor] range:NSMakeRange(0, [titleString2 length])];
    
    [btSignIn setAttributedTitle:titleString2 forState:UIControlStateNormal];
    
    [lblTitle setFont:[UIFont fontWithName:APP_TITLE_FONT size:33]];
    
    [titleString2 release];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(indexPath.row==0){
        ...
        if(indexPath.row==[self tableView:tableView numberOfRowsInSection:0]-1){
            ...
            shape.path = [UIBezierPath bezierPathWithRoundedRect:cell.bounds byRoundingCorners:
                          UIRectCornerTopLeft|UIRectCornerTopRight cornerRadii:CGSizeMake(8, 8)].CGPath;
        ...
        CAShapeLayer *shape = [[CAShapeLayer alloc]init];
        shape.path = [UIBezierPath bezierPathWithRoundedRect:cell.bounds byRoundingCorners:
                      UIRectCornerBottomLeft|UIRectCornerBottomRight cornerRadii:CGSizeMake(8, 8)].CGPath;
        cell.layer.mask = shape;
        cell.layer.masksToBounds = YES;
        [shape release];
    }
    
    UIImageView *seperateLine = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"table_seperate_line.png"]];
    CGRect frame = seperateLine.frame;
    frame.size.height /= 2;
    frame.size.width /= 2;
    seperateLine.frame = frame;
    ...
    if ([indexPath row] == 0) {
        //icon = @"user.png";
        placeholder = NSLocalizedString(@"E-mail or nickname", nil);
